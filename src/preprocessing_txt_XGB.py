...
def get_pipeline():
    ...
    log_std_pipeline = Pipeline([("imputer", SimpleImputer(strategy="median")), ("log1p", FunctionTransformer(np.log1p, validate=False, feature_names_out="one-to-one"))])
    std_pipeline = Pipeline([("imputer", SimpleImputer(strategy="mean"))])
    minmax_pipeline = Pipeline([("imputer", SimpleImputer(strategy="most_frequent"))])
