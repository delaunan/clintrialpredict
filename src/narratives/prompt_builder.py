...
EXPERT_ANALYSIS_REQUIREMENTS = {
    ...
    "evidence_boundaries": [
        ...
        "Use structured Completion Outlook score inputs as Completion Outlook drivers; keep planned enrollment, planned site count, Planned Total Timeline, operational benchmark metadata, and broad operational-footprint wording in Design Confidence only.",
    ...
    "output_examples": {
        ...
        "good_score_design_boundary_comment": (
            "The Completion Outlook improvement should be treated as score-pattern favorability, while Design Confidence should still test whether the current scenario supports the intended decision."
        ),
...
OUTPUT_STYLE_REQUIREMENTS = {
    "general": [
        ...
        "State the main Design Confidence trade-off once, then use subcategory rationales, central tension, and questions for distinct angles.",
        ...
        "Leave Design Confidence points, Total Scenario Score, and subcategory point values to application-owned calculations.",
    ],
    "field_lengths": {
        ...
        "main_tension": "1 sentence stating the central Completion Outlook versus Design Confidence tension",
    ...
    "visible_output_focus": (
        "Write four concise visible sections: Completion Outlook Analysis, Design Confidence Analysis, Main Tension, and two Key Questions. "
        "Use internal subcategories for validation, scoring, and treemap rationale while keeping the visible narrative organized around the four concise sections."
    ),
}
...
def provider_response_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "completion_outlook_rules": {
            ...
            "duration_boundary": (
                "primary_duration_months_ml may be used when it appears as Completion Outlook score evidence; Planned Total Timeline remains Design Confidence context rather than a Completion Outlook driver."
            ),
            "causality_boundary": "Frame field effects as score-pattern hypotheses rather than causal completion or termination claims.",
        },
        "mode_constraints": {
            PROMPT_MODE_HIDDEN_BASELINE: (
                "Create hidden qualitative baseline context only; visible must be false and no baseline Design Confidence comparison is allowed."
            ),
            PROMPT_MODE_FIRST_VISIBLE_ITERATION: (
                "Compare Completion Outlook to the visible original Completion Score while evaluating Design Confidence only for the current visible scenario."
            ),
            PROMPT_MODE_LATER_VISIBLE_ITERATION: (
...
def _mode_instruction(prompt_mode: str) -> str:
    if prompt_mode == PROMPT_MODE_HIDDEN_BASELINE:
        return (
            ...
            "Keep baseline Design Confidence, Total Scenario Score, and hidden numeric design scores out of visible output.\n"
        )
    if prompt_mode == PROMPT_MODE_FIRST_VISIBLE_ITERATION:
        return (
            ...
            "relative to the visible original Completion Score. Evaluate Design Confidence for the current "
            "scenario using supported field changes and evidence only. Use review-panel wording suitable for the simulator Scenario Review panel.\n"
        )
    return (
        ...
        "Use review-panel wording suitable for the simulator Scenario Review panel.\n"
    )
...
def build_provider_prompt(packet: dict[str, Any], *, prompt_mode: str | None = None) -> str:
    ...
    return (
        ...
        "Planning assumptions = planned enrollment, planned sites, and Planned Total Timeline; these fields are Design Confidence context and do not feed the Completion Outlook score. "
        ...
        "Design Confidence evaluates whether the scenario is a coherent, interpretable, patient-relevant, and operationally proportionate design for the intended development decision.\n"
        ...
        "Design Confidence subcategory meanings: Phase & Intent Alignment asks whether phase, purpose, strategic ambition, modality, and evidence ambition fit the development decision. "
        ...
        "If completion_outlook_mode is fixed_planning_assumption_boundary, completion_outlook_analysis.risk_pattern_summary must equal required_completion_outlook_sentence exactly, with any completion_outlook_forbidden_latest_fields reserved for Design Confidence. Use this fixed planning-assumption sentence only for that mode. "
        ...
        "Organize visible content into four concise sections: Completion Outlook Analysis, Design Confidence Analysis, Main Tension, and two Key Questions. "
        "Keep subcategories internal for validation, scoring, and treemap labels. State one concise main_tension, then use each question for a distinct current dilemma. "
        ...
        "Planned Total Timeline / operational_assumptions.planned_duration_months is a planning assumption: it may inform Design Confidence proportionality while Completion Outlook movement uses score-input evidence. "
        "If the latest change is limited to planned enrollment, planned site count, and/or Planned Total Timeline, the Completion Outlook score is unchanged because these fields do not feed it. If text_context or planning assumptions changed but no structured Completion Outlook score input changed, say the score remains stable and keep those details in Design Confidence. If score inputs also changed, explain Completion Outlook using those score-input changes only. "
        ...
        "If score_delta is near zero and no structured Completion Outlook score input changed, keep the prior Completion Outlook storyline stable and state that non-score-input changes belong in Design Confidence. "
        ...
        "In that operational-only zero-delta case, keep extra commentary derived from those three planning assumptions, such as enrollment size, site count, Planned Total Timeline, or broad operational-footprint wording, in Design Confidence only.\n"
        "When structured_features and Trial description fields conflict, apply the same rule: use only the conflicting text for the consistency warning and scenario-readiness discussion. A scenario-readiness warning should usually affect the single most relevant Design Confidence subcategory; multiple strong negative subcategory ratings require independent non-conflicting evidence.\n"
