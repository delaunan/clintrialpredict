"""App-owned Strategic Review storyline state helpers."""
...
def build_storyline_state(validated_review: dict[str, Any] | None) -> dict[str, Any]:
    """Build compact continuity state from a validated Strategic Review."""
    ...
    active_tension = (
        strategic_review.get("current_tension")
    ...
    return {
        ...
        "last_effect_label": str(strategic_review.get("effect_label") or ""),
        ...
        "next_consideration": str(strategic_review.get("next_consideration") or "").strip(),
        "storyline_update": str(continuity.get("storyline_update") or "").strip(),
    }
