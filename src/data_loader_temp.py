...
class ClinicalTrialLoader:
    ...
    def load_and_clean(self):
        ...
        cols_studies = ['nct_id', 'overall_status', 'study_type', 'phase',
                        'start_date', 'start_date_type', 'number_of_arms', 'why_stopped']
        ...
        df = df[df['start_year'].between(2000, current_year)]
    ...
    def add_features(self, df):
        ...
        # 6. Complexity Engineering (FIXED: Includes healthy_volunteers)
    ...
    def _engineer_sponsor_features(self, df):
        print("    -> Engineering Sponsor Tiers & Experience...")

        # FIX: Added 'agency_class' to the columns to load
        ...
        if df_sponsors.empty:
            df['sponsor_tier'] = 'TIER_4_ONE_OFF'
            df['sponsor_experience_log'] = 0
            df['sponsor_clean'] = 'UNKNOWN'
            df['agency_class'] = 'UNKNOWN' # Ensure column exists even if file fails
            return df

        # Filter Lead
        # FIX: Keep 'agency_class' in the subset
        leads = df_sponsors[df_sponsors['lead_or_collaborator'].str.lower() == 'lead'][['nct_id', 'name', 'agency_class']]

        # Rename name -> lead_sponsor
        ...
        # 2. Map Big Pharma (Standardization)
        mappings = {
        ...
        def get_tier(name):
            ...
            c = counts.get(name, 0)
            if c >= 50: return 'TIER_1_GIANT'
            if c >= 10: return 'TIER_2_ESTABLISHED'
            if c >= 2: return 'TIER_3_SMALL'
            return 'TIER_4_ONE_OFF'

        df['sponsor_tier'] = df['sponsor_clean'].apply(get_tier)

        # 4. Experience (Log Count)
    ...
    def _engineer_complexity(self, df):
        ...
        # FIX: Load 'criteria' AND the demographic columns needed by the pipeline
        ...
        # 1. Calculate Complexity (Log Scale)
        df['criteria_len_log'] = np.log1p(df['criteria'].astype(str).str.len().fillna(0))

        # 2. Drop raw text to save memory (but keep the other columns!)
        df.drop(columns=['criteria'], inplace=True, errors='ignore')

        # 3. Fill NaNs for the new columns
    ...
    def _calculate_competition(self, df):
        try:
            ...
            def get_comp(row):
                ...
                return lookup.get((y, area), 0) + lookup.get((y+1, area), 0) + lookup.get((y+2, area), 0)
