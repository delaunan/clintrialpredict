...
DEFAULT_ARTIFACT_PATH = PROJECT_ROOT / "frontend" / "data" / "enrollment_benchmarks_v1.csv"
...
def _empty_metadata(value: Any = None, source: str = "planned_value", hint: str | None = None) -> dict[str, Any]:
    return {
        "planned_enrollment": {
            "value": value,
            "source": source,
...
def load_enrollment_benchmarks(path: str | Path = DEFAULT_ARTIFACT_PATH) -> pd.DataFrame:
    ...
    artifact["low_confidence_flag"] = artifact["low_confidence_flag"].astype(str).str.lower().isin({"true", "1", "yes"})
    return artifact


def classify_enrollment(planned_enrollment: Any, benchmark_row: pd.Series | dict[str, Any]) -> str:
    value = pd.to_numeric(planned_enrollment, errors="coerce")
...
def lookup_enrollment_benchmark(
    ...
    *,
    ...
    benchmarks = load_enrollment_benchmarks(artifact_path) if artifact is None else artifact
    if benchmarks.empty:
        return None
    ...
    if require_confident:
        return lookup_enrollment_benchmark(snapshot, benchmarks, artifact_path=artifact_path, require_confident=False)
    return None
...
def planned_enrollment_metadata(
    ...
    *,
    ...
    numeric_value = pd.to_numeric(planned_enrollment, errors="coerce")
    if pd.isna(numeric_value) or float(numeric_value) <= 0:
        return _empty_metadata(planned_enrollment, source, "Planned enrollment is missing or invalid.")

    row = lookup_enrollment_benchmark(snapshot, artifact=artifact, artifact_path=artifact_path)
    if row is None:
