...
class ClinicalTrialLoader:
    # ... [Keep your existing __init__ and _safe_load methods exactly as they are] ...
    ...
    def load_and_clean(self):
        ...
        if df.empty: raise ValueError("Critical Error: 'studies.txt' failed to load.")
        if 'study_type' in df.columns: df = df[df['study_type'].str.upper() == 'INTERVENTIONAL'].copy()
    ...
    def add_features(self, df):
        ...
        # ... [Keep existing Feature Engineering 1-8] ...
        # 1. Phase
        phase_map = {'PHASE1': 1, 'PHASE1/PHASE2': 2, 'PHASE2': 2, 'PHASE2/PHASE3': 3, 'PHASE3': 3}
        ...
        if not df_countries.empty:
            country_stats = df_countries.groupby('nct_id')['name'].agg(cnt='nunique', includes_us=lambda x: 1 if 'United States' in x.values else 0).reset_index()
            ...
            df['is_international'] = 0; df['includes_us'] = 0
        # 4. Standard Metadata
    ...
    # ... [Keep _engineer_sponsor_features, _engineer_complexity, etc. exactly as is] ...
    def _engineer_sponsor_features(self, df):
        # [Content truncated for brevity, assume existing code matches your file]
        ...
        df['lead_sponsor'] = df['lead_sponsor'].fillna('UNKNOWN'); df['agency_class'] = df['agency_class'].fillna('UNKNOWN')
        ...
        mappings = {'Pfizer': ['pfizer', 'wyeth', 'hospira'], 'GSK': ['glaxo', 'gsk', 'smithkline'], 'Novartis': ['novartis', 'sandoz'], 'AstraZeneca': ['astrazeneca', 'medimmune'], 'Merck': ['merck', 'msd'], 'Roche': ['roche', 'genentech', 'hoffmann'], 'Sanofi': ['sanofi', 'aventis', 'genzyme'], 'J&J': ['johnson & johnson', 'janssen'], 'Bayer': ['bayer', 'monsanto'], 'Boehringer': ['boehringer'], 'BMS': ['bristol-myers', 'squibb', 'celgene'], 'Lilly': ['lilly'], 'Abbott': ['abbott', 'abbvie'], 'Amgen': ['amgen'], 'Takeda': ['takeda', 'shire'], 'Gilead': ['gilead'], 'Novo Nordisk': ['novo nordisk'], 'NIH': ['national cancer institute', 'nci', 'national institutes of health', 'nih'], 'Mayo Clinic': ['mayo clinic']}
        ...
        df['sponsor_tier'] = df['sponsor_clean'].apply(lambda name: 'TIER_1_GIANT' if name in mappings.keys() else 'TIER_2_OTHER')
        return df
    ...
    def _engineer_complexity(self, df):
        ...
        if df_elig.empty:
            ...
            for c in ['gender', 'healthy_volunteers', 'adult', 'child', 'older_adult']: df[c] = 'UNKNOWN'
            return df
    ...
    def _attach_medical_hierarchy(self, df):
        try:
            ...
            df['therapeutic_area'] = 'Unknown'; df['therapeutic_subgroup_name'] = 'Unknown'
        return df
    ...
    def _calculate_competition(self, df):
        try:
            ...
            if not all(col in df.columns for col in req_cols):
                df['competition_broad'] = 0; df['competition_niche'] = 0
                return df
            ...
            df['competition_broad'] = df.apply(lambda row: lookup.get((row['start_year'], row['therapeutic_area']), 0) + lookup.get((row['start_year']-1, row['therapeutic_area']), 0), axis=1)
            if 'therapeutic_subgroup_name' in df.columns:
                ...
                df['competition_niche'] = df.apply(lambda row: lookup_niche.get((row['start_year'], row['therapeutic_subgroup_name']), 0) + lookup_niche.get((row['start_year']-1, row['therapeutic_subgroup_name']), 0), axis=1)
            else:
