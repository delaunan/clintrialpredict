...
TAXONOMY_PATH = PROJECT_ROOT / "models" / "feature_taxonomy.joblib"
...
# ==========================
# HELPER FUNCTIONS (MODEL / BENCHMARK)
# ==========================
...
def calculate_risk_drivers(model, explainer, taxonomy, row_data):
    ...
    try:
        ...
        df_pillars = (
            df_impacts
            .groupby('Pillar', as_index=False)['Impact_Pct']
            .sum()
        )
...
def plot_success_gauge(prob_success):
    """Gauge for completion probability (no internal title, bigger number)."""
    score_val = prob_success * 100

    # Build gradient steps (red -> white -> green)
    steps = []
    ...
    fig = go.Figure(go.Indicator(
        ...
        number={
            "suffix": "%",
            "font": {"size": 34, "color": "black", "family": "Arial"}  # bigger score
        },
        domain={"x": [0, 1], "y": [0, 1]},
        gauge={
            "axis": {
    ...
    fig.update_layout(
        margin=dict(l=15, r=15, t=10, b=10),  # no internal title -> small top margin
        paper_bgcolor="white",
        font={"family": "Arial"},
        height=220,
    )
...
def plot_impact_bar(df_pillars):
    """Compact Bar Chart of pillar impacts (auto-scaled so all bars are visible)."""
    df_sorted = (
        df_pillars
        .dropna(subset=["Impact_Pct"])
        .sort_values("Impact_Pct", ascending=True)
    )
    ...
    # IMPORTANT CHANGE: let Plotly auto-scale x (no manual range)
    fig.update_layout(
        title_text="",                # no internal title
        xaxis_title="",
        yaxis_title="",
        showlegend=False,
    ...
    fig.update_xaxes(
        ...
        showgrid=False,
        zeroline=False,
        # no range=... here so full bar length is visible by default
    )
    fig.update_yaxes(
