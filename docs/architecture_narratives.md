...
- Current setup status: local `.env` can already hold these values, and OpenAI/Gemini keys can be added there, but the narrative provider code does not read them yet. The current implementation still uses the deterministic mock provider. The next implementation step should add `src/narratives/provider_config.py` and `scripts/check_narrative_provider_config.py` to read and validate this configuration without making any LLM API call.
