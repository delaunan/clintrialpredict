...
The active workflow is the two-pass Trial Score workflow. Pass 1 returns structured analytical JSON and rough source-note narrative. The application validates Pass 1, calculates app-owned Trial Score fields, and injects those app scores into Pass 2. Pass 2 returns participant-facing narrative only.
...
The active score views are Completion Outlook, Reality Check, and Trial Score. Completion Outlook remains model-derived. Operational Fit and Reality Check are app-scored adjustments that can be rendered as part of the Trial Score view without being described as SHAP/model drivers.
...
Provider abstraction should be thin. The application should own payload construction, validation, Operational Fit and Reality Check scoring, Trial Score calculation, persistence, cache lookup, and UI rendering. Provider-specific code should own only model invocation and response normalization. The V1 provider boundary includes the deterministic mock provider, explicit unsupported-provider failure path, and real OpenAI/Gemini invocation behind the same normalized result shape.
...
- If Pass 2 JSON is malformed or fails validation after repair, keep the app-calculated Trial Score trace but mark participant narrative unavailable/warning.
