...
- Current setup status: local `.env` can hold these values, and `src/narratives/provider_config.py` reads and validates them without making any LLM API call. The current review implementation still uses the deterministic mock provider; OpenAI and Gemini calls are not wired yet.
- After provider config is validated, add a small OpenAI smoke test through the config path, then add prompt/schema scaffolding, then connect real OpenAI calls, then add Gemini fallback through the same normalized provider result shape.
