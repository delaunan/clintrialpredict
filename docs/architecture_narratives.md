...
- For many low-cost live tests per day, use a single cheap primary provider rather than a fallback chain. The current validated local development profile is Gemini-only with no effective fallback: `NARRATIVE_LLM_PROVIDER=gemini`, `NARRATIVE_LLM_FALLBACK_PROVIDER=gemini`, `GEMINI_NARRATIVE_MODEL=gemini-3.1-flash-lite`, visible Pass 1 `thinking_level=medium`, Pass 2 scoring and Pass 3 narrative `GEMINI_THINKING_LEVEL=medium`, `NARRATIVE_LLM_MAX_OUTPUT_TOKENS=20000`, `NARRATIVE_LLM_TIMEOUT_SECONDS=100`, and `NARRATIVE_LLM_MAX_RETRIES=0`. The config loader collapses same-provider fallback to `None`. Hidden baseline overrides the visible profile with a bounded fast setup profile: Gemini `thinking_level=medium`, `max_output_tokens<=8000`, request timeout capped at 100 seconds, no malformed/MAX_TOKENS retry loop, compact prompt wording, no hidden-baseline word-count minimum, and deterministic compact fallback context if the bounded provider pass fails.
...
- Continue monitoring Gemini's full visible-iteration JSON reliability during representative live play. The current hardening uses SDK response schema, omitted/default temperature, visible Pass 1 `thinking_level=medium`, Pass 2/Pass 3 primary `thinking_level=medium`, a 12000-token minimum schema output ceiling, and one explicit malformed/MAX_TOKENS retry with lower thinking and a 16000-token ceiling, while preserving app-owned validation and scoring.
