...
6. Participants run `Check Scenario` from the Trial Features workflow.
7. If a material structured/text mismatch is found, participants correct the field/text or add a short explanation before prediction proceeds.
8. Participants click `Predict Trial Completion`.
9. XGBoost returns the new completion score, pillar impacts, and impact decomposition through the existing prediction path.
10. The narrative layer receives baseline context, previous prediction, current prediction, changed fields, score deltas, SHAP/pillar movement, operational benchmark metadata, text context, clarification context, and prior storyline memory.
11. The application validates the structured Quality Review, calculates Quality Adjustment and Final Candidate Score, and stores a compact storyline update.
12. The UI displays the Quality Review below or near the score and charts.
13. Participants iterate.
...
Before a new prediction result is committed or displayed, the application should run a structured/text consistency check. The current implementation is a conservative deterministic gate, intentionally small, covering only a few obvious material mismatches such as endpoint-structure, endpoint-rigor, and placebo/control signals that appear inconsistent between Trial Features and editable text. For example, if the structured endpoint rigor is set to hard clinical outcome but the endpoint text clearly describes immunogenicity, antibody titers, seroprotection, biomarker, or surrogate evidence, the prediction workflow should pause before new scoring. The participant either corrects the structured field/text or adds a short explanation. That explanation becomes `clarification_context` in the review packet and is available to the later review LLM as participant scenario context.

The next implementation step should replace or complement the narrow regex-style gate with a dedicated lightweight LLM consistency-check pass before prediction. This pass should compare the current structured Trial Features, display labels, and editable text fields, then return a small structured result: `passed`, `clarification_needed`, or `soft_tension`; issue id; affected fields; text evidence; structured value; confidence; severity; and participant-facing explanation. It must not calculate Completion Score, Quality Adjustment, Final Candidate Score, or decide which field is true. Structured Trial Features remain authoritative for the XGBoost Completion Score; text fields provide rationale, contradiction evidence, or missing-context signals. The LLM consistency pass is a workflow gate, not the Quality Review itself.

The consistency-check LLM should be intentionally narrower than the full Quality Review prompt. It should receive only the fields needed to compare structured values and text, not XGBoost impacts, Quality Assessment rubric, baseline review memory, or full scoring context. This keeps the check fast, cheaper, and easier to explain. It should run from the `Check Scenario` action in the Trial Features workflow and before `Predict Trial Completion` is enabled. If it returns `clarification_needed`, the participant should fix the field/text or add an explanation; if it returns `soft_tension`, prediction may proceed and the issue can be passed into the later Quality Review as narrative context. Deterministic rules can remain as a safety net for obvious high-confidence cases, but should not keep expanding into a broad regex NLP system.
...
- Provider config, prompt/schema fixtures, opt-in OpenAI/Gemini smoke testing, and opt-in simulator UI routing are implemented. If live routing is enabled and all configured providers fail or validation does not produce a complete Quality Review, the participant panel shows Completion Score only and marks Quality Review unavailable for the current scenario, with a narrow retry action for live-provider failures. It does not reuse a stale Quality Adjustment. The next planning step is the lightweight LLM consistency-check pass before revisiting Quality Assessment display/pillars.
...
2. Pre-prediction structured/text consistency check. Before revisiting the Quality Assessment pillars, define the lightweight LLM pass that checks whether editable text fields and categorical/numeric Trial Features tell the same scenario. The objective is to make participants resolve material mismatch before a Completion Score is committed, without letting the LLM rewrite structured truth or score the trial. This step should specify the small consistency-check packet, JSON response contract, confidence/severity thresholds, UI states for `Check Scenario`, handling of user explanations, and cache/replay behavior. The existing deterministic placebo/endpoint rules are a temporary safety net and fixture source, not the long-term approach.
3. Quality Adjustment and participant display review. After the consistency-check workflow is defined, reassess whether the Quality Assessment pillars, domain labels, rating-to-point mapping, and participant wording are coherent, legitimate, and easy to explain. The review should test whether participants can understand how Completion Score features, consistency-check warnings, Quality Review evidence, and Quality Adjustment differ, and whether the current grouped contribution chart is enough or whether a separate gauge, separate contribution panel, or two-branch treemap is clearer.
4. Implementation plan for prompt/UI changes. Only after the first three reviews decide what to change in code: which packet fields should be removed, summarized, or added; whether hidden baseline generation should be deferred or cached durably; which provider/model profile should be used for interactive play; how the consistency-check pass should be invoked and cached; and how the Final Candidate Score, Completion Score drivers, and Quality Adjustment contributions should be rendered in the simulator.
...
- The simulator now has a `Check Scenario` action in the Trial Features workflow. The intended interaction is: participant edits Trial Features/text, runs the consistency check, resolves material mismatch or adds explanation, then runs `Predict Trial Completion`.
- The implemented check is still deterministic and deliberately narrow. It catches selected high-confidence endpoint and placebo/control mismatches and stores participant explanations as clarification context. It is not the target long-term mechanism.
- The next work item is to design and implement a dedicated LLM consistency-check pass using the selected Gemini Flash-Lite profile. This pass should be much smaller than the full Quality Review prompt and should run before scoring. It should return structured issue objects with confidence/severity and distinguish `clarification_needed` from `soft_tension`.
- The consistency-check result should be cached by a hash of structured Trial Features, display labels, editable text context, and user clarifications. It should not include Completion Score, XGBoost impacts, hidden baseline review, Quality Adjustment rubric, or prior narrative memory unless later evidence shows they are necessary.
- The output of this pass should feed the existing clarification UI and later Quality Review context, but should not directly affect XGBoost scoring or Quality Adjustment math. Material unresolved mismatches pause prediction; resolved explanations are carried into `clarification_context`.
...
- Current real-provider traces store prompt template version, response schema version, configured/applied generation controls, attempts, latency, parse status, response text length, token usage when available, finish metadata when available, malformed-JSON retry metadata, and fallback-after metadata in provider metadata. The UI may expose these fields in a compact technical diagnostics expander when live Quality Review is unavailable, without showing API keys, raw prompts, or raw provider output. Add prompt template hashes only if prompt version strings are not enough for audit. Add an explicit pre-review gate status, such as `passed` or `clarification_needed`, if clarification debugging becomes ambiguous. Add a compact evidence-audit summary only if unsupported evidence fields become hard to inspect from `quality_assessment`.
...
6. Pre-prediction consistency check: before committing a new Completion Score, compare editable text fields and structured Trial Features. Current implementation artifact: `frontend/views/trial_simulator.py` plus `src/narratives/alignment.py`, validated by `scripts/check_narrative_alignment.py`, provides a narrow deterministic gate and Trial Features `Check Scenario` workflow. Next implementation target: replace or complement this with a lightweight LLM consistency-check provider path and dedicated response contract, using Gemini Flash-Lite settings, small input packet, confidence/severity thresholds, and cached structured issue results.
