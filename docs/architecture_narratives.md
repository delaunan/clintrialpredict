...
- Current setup status: local `.env` can hold these values, and `src/narratives/provider_config.py` reads and validates them without making any LLM API call. `scripts/check_narrative_openai_smoke.py` can run an opt-in OpenAI API smoke test when `RUN_NARRATIVE_OPENAI_SMOKE=1` is set; it skips by default to avoid accidental network calls or API spend. The current review implementation still uses the deterministic mock provider; OpenAI and Gemini review calls are not wired yet.
- After provider config and optional OpenAI smoke testing are validated, add prompt/schema scaffolding, then connect real OpenAI calls, then add Gemini fallback through the same normalized provider result shape.
...
- Runtime controls such as temperature and seed are provider/model-specific. Config may read them, but real provider code should send only parameters supported by the selected model. The initial OpenAI smoke test for the pinned GPT-5.5 snapshot uses the configured model and max output token cap, but does not send temperature because that model rejects the parameter.
...
8. Thin LLM provider wrapper: add provider config first, then an opt-in config-path API smoke test, then prompt/schema scaffolding, then real OpenAI/Gemini provider calls only after packet building, validation, scoring, caching, replay, mock UI work, and deterministic clarification gating. Provider config reads API keys from environment variables or secret managers; it never stores keys in code. Current implementation artifacts: `src/narratives/provider.py` and `src/narratives/provider_config.py`, validated by `scripts/check_narrative_provider.py`, `scripts/check_narrative_provider_config.py`, and the skipped-by-default `scripts/check_narrative_openai_smoke.py`; the simulator still uses the deterministic mock provider. Provider code invokes the model and normalizes JSON only; it should not decide field truth when structured/text inputs are asynchronous. The application owns scoring and returns `clarification_needed` before new scoring/provider review for unresolved material mismatches.
