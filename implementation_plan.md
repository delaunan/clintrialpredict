...
Final boundary validation checkpoint is frozen without another live rerun. The latest completed live boundary run, `final_validation_boundary_3_4`, returned 33/33 reviewed visible iterations, 6 failed checks, and 3 warnings. Accepted residuals are participant question direct-address wording and internal model-language leaks. The substantive shortcut calibration issue was fixed, and the remaining mixed structured-plus-planning proxy phrase issue was addressed surgically after the run by extending `structured_score_inputs_only` to exclude planning-assumption proxy phrases such as `operational footprint`, `operational scale`, `site expansion`, `larger enrollment`, `scaled execution`, and `site performance`. Local verification passed; next work should move to the larger validation/storyline plan or commit this checkpoint as the current narrative-control baseline.
