...
def update_production_reports_expanded():
    ...
    # Using raw strings for the problematic lines to avoid escape confusion
    header_line = r'header = f"{row["TA"]:<20} | {row["N Hist"]:<6} | {row["Fail% Hist"]:<7} | {row["AUC Hist"]:<6} | {row["Prec Hist"]:<6} | {row["Rec Hist"]:<6} | {row["N Rec"]:<6} | {row["Fail% Rec"]:<7} | {row["AUC Rec"]:<6} | {row["Prec Rec"]:<6} | {row["Rec Rec"]:<6} | {row["Thresh"]}"'
    # Actually let's just use a very simple approach: write the whole source as a single raw string
...
header = f"{row['TA']:<20} | {row['N Hist']:<6} | {row['Fail% Hist']:<7} | {row['AUC Hist']:<6} | {row['Prec Hist']:<6} | {row['Rec Hist']:<6} | {row['N Rec']:<6} | {row['Fail% Rec']:<7} | {row['AUC Rec']:<6} | {row['Prec Rec']:<6} | {row['Rec Rec']:<6} | {row['Thresh']}"
print(header)
print('-' * len(header))
...
print('\n>>> RENDERING TOP 6 THERAPEUTIC AREA DEEP DIVES (RESTORED FIDELITY) ...')"""
    ...
    for cell in nb.cells:
        if cell.cell_type == 'code' and target_marker in cell.source:
            ...
            print("Successfully updated Step 13 with expanded metrics and sorting.")
            break
...
if __name__ == "__main__":
    update_production_reports_expanded()